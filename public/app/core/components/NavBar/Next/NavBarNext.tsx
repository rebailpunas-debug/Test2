...
import { GrafanaTheme2, NavModelItem, NavSection, OrgRole } from '@grafana/data';
...
export const NavBarNext = React.memo(() => {
  ...
  // Filter out alerting for viewer role
  const isViewer = contextSrv.user?.orgRole === OrgRole.Viewer;
  const filteredNavTree = navTree.filter((item) => {
    if (isViewer && item.id === 'alerting') {
      return false;
    }
    return true;
  });

  const coreItems = filteredNavTree.filter((item) => item.section === NavSection.Core);
  const pluginItems = filteredNavTree.filter((item) => item.section === NavSection.Plugin);
  const configItems = enrichConfigItems(
    filteredNavTree.filter((item) => item.section === NavSection.Config),
    location,
    toggleSwitcherModal
  );
  const activeItem = isSearchActive(location) ? searchItem : getActiveItem(filteredNavTree, location.pathname);
