...
export const SignupInvitedPage: FC<Props> = ({ match }) => {
  ...
  const onSubmit = async (formData: FormModel) => {
    await getBackendSrv().post('/api/user/invite/complete', {
      ...formData,
      inviteCode: code,
    });
    ...
    <Page navModel={navModel}>
      <Page.Contents>
        ...
        <div className="modal-tagline p-b-2">
          <em>{invitedBy || 'Someone'}</em> has invited you to join AKMind and the organization{' '}
          <span className="highlight-word" style={{ color: '#52C1C4' }}>{contextSrv.user.orgName}</span>
          <br />
